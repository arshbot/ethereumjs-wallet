...
var fixturePrivKey = 'efca4cdd31923b50f4214af5d2ae10e7ac45a5019e9431cc195482d707485378'
var fixturePrivKeyStr = '0x' + fixturePrivKey
...
var fixturePubKeyBuffer = Buffer.from(fixturePubKey, 'hex')

var fixtureWallet = Wallet.fromPrivateKey(fixturePrivKeyBuffer)

describe('.getPrivateKey()', function () {
  it('should work', function () {
    assert.equal(fixtureWallet.getPrivateKey().toString('hex'), fixturePrivKey)
  })
...
describe('.getPrivateKeyString()', function () {
  it('should work', function () {
    assert.equal(fixtureWallet.getPrivateKeyString(), fixturePrivKeyStr)
  })
})

describe('.getPublicKey()', function () {
  it('should work', function () {
    assert.equal(fixtureWallet.getPublicKey().toString('hex'), fixturePubKey)
  })
})

describe('.getPublicKeyString()', function () {
  it('should work', function () {
    assert.equal(fixtureWallet.getPublicKeyString(), fixturePubKeyStr)
  })
})
...
describe('public key only wallet', function () {
  var pubKey = Buffer.from(fixturePubKey, 'hex')
...
describe('raw new Wallet() init', function () {
  it('should fail when both priv and pub key provided', function () {
    assert.throws(function () {
      new Wallet(fixturePrivKeyBuffer, fixturePubKeyBuffer) // eslint-disable-line
